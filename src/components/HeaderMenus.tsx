  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <div className="inline-block relative">
        <button onClick={() => setOpen(true)} aria-label="Notificaciones" className="inline-flex h-12 w-12 items-center justify-center rounded-full">
          <Bell className={`w-5 h-5 ${nearCount > 0 ? 'text-accent' : 'text-muted-foreground'}`} />
          {nearCount > 0 && (
            <span
