  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <div className="inline-block relative">
        <button onClick={() => setOpen(true)} aria-label="Notificaciones" className="inline-flex h-12 w-12 items-center justify-center rounded-full hover:bg-black/5 dark:hover:bg-white/10 transition-colors focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
          <Bell className={`w-6 h-6 ${nearCount > 0 ? 'text-accent' : 'text-muted-foreground'}`} />
          {nearCount > 0 && (
            <span
