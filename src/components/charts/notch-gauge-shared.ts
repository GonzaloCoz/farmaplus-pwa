...
export function interpolateGaugeHex(
  ...
  // If color1 is red and color2 is green, interpolate via yellow in the middle
  if (c1 === "#ef4444" && c2 === "#22c55e") {
    const yellow = "#eab308"; // Tailwind yellow-500
    if (factor < 0.5) {
      return interpolateHex(c1, yellow, factor * 2);
    } else {
      return interpolateHex(yellow, c2, (factor - 0.5) * 2);
    }
  }

  return interpolateHex(c1, c2, factor);
}
