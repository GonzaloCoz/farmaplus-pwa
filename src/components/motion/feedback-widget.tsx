...
export function FeedbackWidget({
  ...
  placeholder = "Comparte una idea o reporta un problema...",
  icon,
  className,
}: FeedbackWidgetProps) {
  ...
  return (
    ...
    >
      ...
      <motion.div
        layout
        animate={{ borderRadius: open ? 20 : 12 }}
        transition={surfaceTransition}
        style={{ transformOrigin: left ? "bottom left" : "bottom right" }}
        className={cn(
          "pointer-events-auto absolute bottom-0 overflow-hidden text-foreground transition-all duration-300",
          open
            ? "w-[min(86vw,320px)] p-2 border border-border bg-background shadow-lg"
            : "group flex items-center justify-center h-10 w-10 transition-all duration-300 rounded-xl outline-none border border-transparent ring-0 bg-transparent text-muted-foreground/80 hover:bg-muted hover:text-foreground",
          left ? "left-0" : "right-0",
        )}
      >
                    ...
                    >
                      <div className="flex flex-col items-center justify-center gap-1.5 rounded-[20px] bg-border/60 px-4 py-6 text-center">
                        ...
                        <p className="text-xs leading-relaxed text-muted-foreground">
                          Tus comentarios nos ayudan a crear una mejor herramienta.
                        </p>
                      </div>
                    </motion.div>
                  ) : status === "error" ? (
