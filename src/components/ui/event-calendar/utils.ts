...
export function sortEvents(events: CalendarEvent[]): CalendarEvent[] {
  return [...events].sort((a, b) => {
    ...
    return safeParseDate(a.start).getTime() - safeParseDate(b.start).getTime();
  });
}
 ...
 */
export function safeParseDate(dateInput: any, fallback: Date = new Date()): Date {
  ...
  if (dateInput instanceof Date) {
    ...
    // Try parseISO first (for ISO strings), then standard Date constructor
...
export function addHoursToDate(date: Date, hours: number): Date {
  const result = new Date(safeParseDate(date));
