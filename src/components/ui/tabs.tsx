...
const TabItem = forwardRef<HTMLButtonElement, TabItemProps>(
  ({ value, icon: Icon, label, children, _index = 0, className, onClick, ...props }, ref) => {
    ...
    // Extract any icon passed inside children dynamically
    const childrenArray = Children.toArray(children);
    let resolvedIcon = Icon;
    let resolvedLabel: ReactNode = label;

    if (!resolvedIcon && childrenArray.length > 0) {
      const first = childrenArray[0];
      if (isValidElement(first) && typeof first.type !== "string") {
        resolvedIcon = first.type as any;
        resolvedLabel = label ?? childrenArray.slice(1);
      }
    }
    if (!resolvedLabel) {
      resolvedLabel = label ?? children;
    }
...
const TabsTab = TabItem;
const TabsPanel = TabPanel;
const TabsContent = TabPanel;
const TabsTrigger = TabItem;

export { Tabs, TabsList, TabItem, TabPanel, TabsTab, TabsPanel, TabsContent, TabsTrigger };
