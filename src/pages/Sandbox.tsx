...
export default function Sandbox() {
  return (
    <PageLayout>
      <PageHeader
        title="Sandbox Coss UI"
        subtitle="Validación del componente p-select-20 de forma aislada."
      />
      <div className="max-w-md mx-auto mt-8 p-6 bg-card rounded-lg border border-border shadow-sm">
        <Select
          aria-label="Select user"
          defaultValue={users[0]}
          itemToStringValue={(item) => item.value}
        >
          <SelectTrigger className="h-auto py-1.5 w-full">
            <SelectValue>
              {(item) => (
