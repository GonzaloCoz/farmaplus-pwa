-- ==========================================
-- RPC: Recomputar progreso con GUARD para Auto-Archive
-- Evita que el progreso baje a 0% si el laboratorio está vacío (mesa limpia)
-- ==========================================
...
BEGIN
  ...
  -- 2. Contar TODOS los items del laboratorio (incluyendo pendientes)
  SELECT COUNT(*) INTO v_total_in_inventories
  FROM public.inventories
  WHERE branch_name ILIKE TRIM(p_branch_name)
    AND laboratory ILIKE TRIM(p_laboratory);

  -- [GUARD] Si no hay items en inventarios (mesa de trabajo limpia), 
  -- NO actualizamos branch_laboratories para preservar el 100% o avance previo.
