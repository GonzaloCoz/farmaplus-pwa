-- ========================================================
-- Migración: Limpieza de Pendientes y Finalización Ferrari
-- Fecha: 2026-03-13
...
BEGIN
  -- 1. Contar controlados esperando cierre
  ...
  -- 2. Si hay algo para ajustar, lo procesamos
  IF v_controlled_count > 0 THEN
      -- "El Snap": Pasar a ajustado y apilar IDs si ya existen
          ...
          adjustment_id_shortage = CASE 
            WHEN quantity < system_quantity AND NULLIF(p_shortage_id, '') IS NOT NULL THEN 
              CASE 
                ...
                WHEN adjustment_id_shortage ~ ('(^|,)' || p_shortage_id || '($|,)') THEN adjustment_id_shortage 
          ...
          adjustment_id_surplus = CASE 
            WHEN quantity > system_quantity AND NULLIF(p_surplus_id, '') IS NOT NULL THEN 
              CASE 
                ...
                WHEN adjustment_id_surplus ~ ('(^|,)' || p_surplus_id || '($|,)') THEN adjustment_id_surplus 
