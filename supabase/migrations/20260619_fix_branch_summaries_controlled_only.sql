-- ========================================================
-- FIX: branch_summaries VIEW — Solo items controlled/adjusted (ciclo completo)
-- Fecha: 2026-06-19
-- Problema: positive/negative_diff_units sumaban items pending,
--   mostrando valores falsos en sucursales sin ajustes reales.
-- Solución: Filtrar solo por status IN ('controlled','adjusted').
--   SIN filtro de mes: el Monitor de Sucursales muestra el ciclo
--   completo (puede abarcar varios meses). El filtro de mes solo
--   aplica en TrendsChartWidget via la RPC get_branch_current_month_metrics.
-- ========================================================
...
branch_inv_metrics AS (
    -- 1. Métricas del ciclo completo (Solo items controlled/adjusted, sin pending)
    SELECT 
        LOWER(TRIM(i.branch_name)) as branch_key,
        SUM(i.quantity) FILTER (WHERE i.status IN ('controlled', 'adjusted')) as inventory_units,
        -- ponytail: only controlled/adjusted — exclude pending false diffs
        SUM(
...
COMMENT ON VIEW public.branch_summaries IS 'Vista en tiempo real para el Monitor de Sucursales. Solo agrega items con status controlled o adjusted para evitar falsos positivos/negativos de items pending.';
