-- Migration: Fix Monitor Timeout and Accent Spellings in Summaries RPC
-- Date: 2026-07-17
-- Purpose: Optimize performance to resolve statement timeouts using CTE joins instead of O(N) subqueries, and strip accents from category names.
...
BEGIN
  ...
  WITH 
  branch_rounds AS (
      -- Pre-resolve rounds for all branches/categories to avoid running subqueries O(N) times
      SELECT 
          LOWER(TRIM(inv_cfg.branch_name)) as branch_key,
  ...
  branch_inv_metrics AS (
      ...
      LEFT JOIN branch_rounds br_cat ON LOWER(TRIM(i.branch_name)) = br_cat.branch_key 
        AND TRANSLATE(UPPER(TRIM(COALESCE(i.category, 'Varios'))), 'ÁÉÍÓÚ', 'AEIOU') = br_cat.category_key
      LEFT JOIN branch_rounds br_gen ON LOWER(TRIM(i.branch_name)) = br_gen.branch_key 
  ...
  lab_averages AS (
      ...
      LEFT JOIN branch_rounds br_cat ON LOWER(TRIM(bl_sub.branch_name)) = br_cat.branch_key 
        AND TRANSLATE(UPPER(TRIM(COALESCE(bl_sub.category, 'Varios'))), 'ÁÉÍÓÚ', 'AEIOU') = br_cat.category_key
      LEFT JOIN branch_rounds br_gen ON LOWER(TRIM(bl_sub.branch_name)) = br_gen.branch_key 
        AND br_gen.category_key = 'GENERAL'
  ...
  branch_lab_metrics AS (
      ...
      LEFT JOIN branch_rounds br_cat ON LOWER(TRIM(bl.branch_name)) = br_cat.branch_key 
        AND TRANSLATE(UPPER(TRIM(COALESCE(bl.category, 'Varios'))), 'ÁÉÍÓÚ', 'AEIOU') = br_cat.category_key
      LEFT JOIN branch_rounds br_gen ON LOWER(TRIM(bl.branch_name)) = br_gen.branch_key 
        AND br_gen.category_key = 'GENERAL'
