-- Migration: Support custom round queries and fix general CONFIG_ROUND fallback in cyclic RPC functions
-- Date: 2026-07-18

-- 1. Drop existing get_lab_inventory_v2 signature
DROP FUNCTION IF EXISTS public.get_lab_inventory_v2(TEXT, TEXT) CASCADE;
DROP FUNCTION IF EXISTS public.get_lab_inventory_v2(TEXT, TEXT, INTEGER) CASCADE;

-- 2. Create updated get_lab_inventory_v2 with optional p_round
...
BEGIN
    IF p_round IS NOT NULL THEN
        v_round := p_round;
    ELSE
        -- Find category of lab to resolve its active round
        ...
        -- Resolve active round falling back to GENERAL CONFIG_ROUND
        v_round := COALESCE(
          (
...
GRANT EXECUTE ON FUNCTION public.get_lab_inventory_v2(TEXT, TEXT, INTEGER) TO anon, authenticated, service_role;


-- 3. Recreate save_cyclic_inventory_v2 to support CONFIG_ROUND fallback
...
BEGIN
  ...
  LOOP
    ...
    IF p_laboratory = '_CONFIG_' THEN
      v_round := 1;
    ELSE
      v_category := COALESCE(item->>'category', 'Varios');
      ...
      v_round := COALESCE(
        (
          ...
          WHERE normalize_string_sql(branch_name) = v_branch 
            ...
            AND ean = 'CONFIG_ROUND_' || UPPER(TRIM(v_category))
        ),
        (
          SELECT quantity::INTEGER 
          FROM public.inventories 
    ...
    FROM public.inventories
    WHERE normalize_string_sql(branch_name) = v_branch AND laboratory = p_laboratory AND ean = (item->>'ean') AND round = v_round;
